 ...
 * <p/>
 * 功能详细描述:
 * From http://blog.kainaodong.com/?p=42
 */
public class DampingInterpolator implements Interpolator {
    ...
    public void setOverShootPercent(float overshoot) {
        ...
        /*
         * 当 t * mRegion = Math.PI 的时候，达到第一次过冲的峰值， 则 t = Math.PI / mRegion 。
         * 且此时 mOvershootModulus^t = mOvershootPercent ，
         * 所以 mOvershootModulus = Math.pow(mOvershootPercent, 1 / t) ，
         * 即 mOvershootModulus = Math.pow(mOvershootPercent, mRegion / Math.PI) 。
         */
        mOvershootModulus = (float) Math.pow(mOvershootPercent, mRegion / Math.PI);
    }
    ...
    public float getInterpolation(float t) {
        ...
        return (float) (1 - Math.pow(mOvershootModulus, t) * Math.cos(mRegion * t));
    }
}
