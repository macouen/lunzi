...
public class PhysicsViewDragHelper {
     ...
     */
    private PhysicsViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
     ...
     */
    public static PhysicsViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new PhysicsViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static PhysicsViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final PhysicsViewDragHelper helper = create(forParent, cb);
    ...
    public boolean continueSettling(boolean deferCallbacks) {
        if (mDragState == STATE_SETTLING) {
            ...
            final int dx = x - (int) mCapturedView.getX();
            final int dy = y - (int) mCapturedView.getY();
        ...
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= child.getX() && x + scrollX < child.getX() + child.getWidth() &&
                        ...
                        canScroll(child, true, dx, dy, x + scrollX - (int) child.getX(),
                                y + scrollY - (int) child.getY())) {
                    return true;
                }
            }
        }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dx != 0) {
            ...
            mCapturedView.setX(left);
        }
        if (dy != 0) {
            clampedY = mCallback.clampViewPositionVertical(mCapturedView, top, dy);
            //mCapturedView.offsetTopAndBottom(clampedY - oldTop);
            mCapturedView.setY(top);
        }
